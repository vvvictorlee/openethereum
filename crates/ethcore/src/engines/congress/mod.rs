...
impl Engine<EthereumMachine> for Congress {
    ...
    fn on_close_block(&self, _block: &mut ExecutedBlock) -> Result<(), Error> {
        let txs = &_block.transactions;//.into_iter().map(|v| v.as_unsigned().tx()).collect::<Vec<_>>();
        ...
        for tx in txs {
            if is_system_transaction(tx, header.author()) {
                actual_system_txs.push(tx.as_unsigned().tx());
            }
        }
        if header.number() == 1 {
        ...
        for i in 0..txs.len() {
            let tx = txs.get(i).unwrap().as_unsigned().tx();
    ...
    fn verify_block_basic(&self, header: &Header) -> Result<(), Error> {
        ...
        let mixhash: H256 = H256::from_slice(seal_fields[0]);//.into();
